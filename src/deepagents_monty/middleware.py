...
- Sync-only backend contract (see DeepAgentBackendOS docstring for the async
  alternative via ``external_functions``).
"""
...
def make_execute_python(
    *,
    ...
    async def execute_python(code: str, runtime: ToolRuntime) -> str:
        ...
        try:
            m = Monty(code, type_check=type_check)
...
class MontyCodeMiddleware(AgentMiddleware):
    ...
    def __init__(
        ...
        *,
        ...
        self._system_prompt = system_prompt if system_prompt is not None else _MONTY_SYSTEM_PROMPT
